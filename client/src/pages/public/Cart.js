...
const Cart = ({ dispatch }) => {
  const { isLoggedIn, currentUser, currentCart } = useSelector(
          ...
          <div className="grid grid-cols-10 py-[15px] px-[20px] border-x border-b overflow-auto">
            ...
            <div className="col-span-3">
              <div className="grid grid-cols-5 mb-3">
                ...
                <span className="text-xl font-semibold text-[#333333] col-span-2 text-end">{`${fomantMoney(
                  currentUser?.cart?.reduce(
                    (sum, el) => (sum += +el.priceChanged),
                    0
                  )
                )} VND`}</span>
              </div>
              <p className="text-[#505050] mb-3 italic text-end">
                Shipping, taxes, and discounts calculated at checkout.
              </p>
              <div className="flex justify-end gap-3 text-sm">
                <button
                  onClick={() => handleUpdateCart()}
                  className="flex items-center gap-2 relative justify-center text-white bg-black py-[11px] px-[15px] rounded"
                >
                  Updated Cart
                  <ClipLoader
                    color={"#fff"}
                ...
                <Link
                  to={`/${path.CHECKOUT_URL}`}
                  className="flex items-center gap-2 text-white bg-main py-[11px] px-[15px]"
                >
                  <span>Check out</span>
                  <FaArrowRightLong />
                </Link>
