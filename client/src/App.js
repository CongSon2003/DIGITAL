...
function App() {
  ...
  const { isShowModel, dataModel, isShowRightCart } = useSelector(
    ...
    <div className="relative">
      {
        <>
          <div
            onClick={() => dispatch(showRightCart({ isShowRightCart: 3 }))}
            className={`absolute inset-0 bg-[#0000004D] z-40 ${
      ...
      <Routes>
        ...
        <Route path={Path.ADMIN_URL} element={<AdminLayout />}>
          ...
          <Route
            path={Path.MANAGER_PRODUCTS_URL}
          ...
          <Route
            path={Path.PRODUCT_VARIATIONS}
            element={<ProductVarriantions />}
          />
        </Route>
        <Route path={Path.MEMBER_URL} element={<UserLayout />}>
          ...
          <Route
            path={Path.PURCHASE_HISTORY_URL}
