  ...
  return (
    <div className='w-full mt-[-20px]'>
      <div className='flex gap-5 max-md:flex-col'>
        <div onMouseMove={() => setArrayIsHover(prev => prev.map(item => item.id === 1 ? {...item, value : 2} : item))} onMouseLeave={() => setArrayIsHover((prev) => prev.map(item => item.id === 1 ? {...item, value : 3} : item))} className='md:w-1/2 flex relative'> 
        ...
        <div onMouseMove={() => setArrayIsHover(prev => prev.map(item => item.id === 4 ? {...item, value : 2} : item))} onMouseLeave={() => setArrayIsHover((prev) => prev.map(item => item.id === 4 ? {...item, value : 3} : item))} className='md:w-1/4 relative'> 
          <img src='https://digital-world-2.myshopify.com/cdn/shop/files/banner4-bottom-home2_92e12df0-500c-4897-882a-7d061bb417fd_400x.jpg?v=1613166661' className='w-full object-cover cursor-pointer' alt=''/>
